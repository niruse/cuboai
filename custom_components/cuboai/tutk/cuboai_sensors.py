...
def _live_value(cache, key, value, *, unit=None, now=None, note=None) -> Reading:
    ...
    if value is not None:
        cache[key] = (value, now)
        return Reading(value=value, source="live", ts_utc=now, age_s=0.0, available=True, unit=unit, note=note)
    hit = cache.get(key)
    if hit is None:
        return Reading(
            value=None,
            ...
            available=False,
            unit=unit,
            note=note or "no successful reading yet",
        )
    cval, cts = hit
    ...
    return Reading(
        ...
        unit=unit,
        stale=True,
        note=note or "last-good value; most recent read failed",
    )
...
def get_live_sensors(sess, *, cache: Optional[dict] = None) -> LiveSensors:
    ...
    if cry or cough or ss_setting:
        detection_val = {
            ...
            "sleep_safety_enabled": (
                bool(ss_setting.get("safety_alert")) if ss_setting.get("safety_alert") is not None else None
            ),
        }
    ...
    return LiveSensors(
        ...
        baby_presence_alert_configured=_live_value(
            ...
            ss_setting.get("baby_presence_alert"),
            now=now,
...
def _wrap_history_reading(value, ts, age, stale, *, unit=None, note=None) -> Reading:
    return Reading(
        value=value, source="history", ts_utc=ts, age_s=age, available=True, unit=unit, stale=stale, note=note
    )
...
def _history_sensors_from_fields(*, bp, na, mo, bw, be, pr, temp, hum, ts, age, stale, fetched_at):
    ...
    return HistorySensors(
        ...
        noise=_wrap_history_reading(
            na, ts, age, stale, unit="0-100", note="elevated (>=60)" if (na is not None and na >= 60) else None
        ),
...
def _history_sensors_unavailable(now) -> HistorySensors:
    def U():
        return Reading(
            value=None, source="history", ts_utc=None, age_s=None, available=False, note="no manifest retrievable yet"
        )
...
def _record_to_point(rec, now) -> HistoryPoint:
    ...
    return HistoryPoint(
        ts_utc=ts,
        age_s=age,
        baby_present=f.get("bp"),
        noise=f.get("na"),
        motion=f.get("mo"),
        wellbeing=f.get("bw"),
        baby_event=f.get("be"),
        privacy=f.get("pr"),
        temperature_c=rec.temp,
...
def get_history_sensors(
    ...
    *,
    ...
    if _pace_allows(cache, _HIST_CACHE_LATEST_GATE, min_pull_interval_s):
        ...
        if rec is not None:
            # A transient growing-hour failure makes _pull_latest_record fall
            # back to the tail of a COMPLETED hour — a "successful" pull of a
            # record that can be much OLDER than the one already cached (at
            # :20 past the hour it is 20 minutes old, past every freshness
            # cutoff). Observed live as the history entities blinking
            # unavailable for one cycle every few minutes. Newest record wins,
            # both for what we serve and for what we cache.
            cached = cache.get(_HIST_CACHE_LATEST)
            if cached is not None and int(getattr(cached, "ts", 0)) > int(rec.ts):
